    ...
    return (
        <form onSubmit={handleSubmit}>
            <label id="thumbnail" style={{backgroundImage:`url(${preview})`}} className={thumbnail?'has-thumbnail':''}>
                <input type="file" name="" onChange={event=>setThumbnail(event.target.files[0])} />
            ...
            <input type="text" id="price" placeholder="Sua empresa" value={price} onChange={event=>setPrice(event.target.value)} />

            <button type="submit" value="Salvar" className="btn">
                Cadastrar
            </button>
        </form>
    );
